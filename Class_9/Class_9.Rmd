...
If you want to get all rows or columns, it's enough to put nothing instead of the respective coordinate, e. g. `[, column_number]`
...
The simplest data type in R is **logical**, with only two values: `TRUE` and `FALSE`. It's simple, but powerful and versatile, as we'll see in a moment. Logical values, as any other basic data type in R can be combined into vectors.
...
1. Condition generates a logical vector of the length equal to that of the original vector (the one you want to subset); in this logical vectors `TRUE` is in positions of the original vector that fulfill the condition and `FALSE` is in the remaining positions.  
