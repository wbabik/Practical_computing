...
Linux is a free operating system that, apart from working in a graphical environment (GUI), allows very efficient work in the command line mode (in a terminal) using Shell. The Shell is the part of the operating system that is responsible for how the terminal looks and functions and how the system executes commands. The most commonly used type of Shell is bash (Bourne again shell). Linux and Linux-like systems are ubiquitous in scientific computing, including computer clusters that are more and more usde in various areas of biology. The working knowledge of Linux is thus an important part of a biologist's toolbox.

So:
- The Terminal is a window where you can enter commands to be executed by the computer - this can be your own computer or a ramote machine you connect to and work on.
...
- `-l`: how the task is to be performed. In the example, the `-l` option tells `ls` to show not only the names of files and directories, but also some additional information about them (who owns them, what is their size, when were they last modified).
...
One solution to this task takes you to type about 6 short lines of text in the terminal, and an average computer will complete the tast for you in couple of seconds.  We will return to this task at the end of the course.
...
We assume that you work on a computer running Windows. If you are running Linux or MacOS, see below. Windows uses different shell from Linux, and we will not cover it during our course. Our first task will be to connect to a remote computer that runs Linux and run a terminal session which we will use for the rest of the class.  To do this, we will use the PuTTy program which you can downlowad [here](https://www.putty.org/).
